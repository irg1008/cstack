...
If a phase creates or edits a skill, the phase instructs the implementer to use the **create-skill** skill (Claude Code's built-in for authoring SKILL.md files).
...
- Browser / Electron / Web UIs: the project's `verify-*` skill (see **create-verification-skill**).
- CLIs and TUIs: the project's `verify-*` skill (see **create-verification-skill**).
...
- Claude Code's built-in **babysit** skill after opening the PR.
