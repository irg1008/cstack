...
This skill orchestrates three others: an inline mining pass (see step 1), Claude Code's built-in `create-skill` (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Use Claude Code's built-in `create-skill` skill to author the skill. Placement:
...
- Claude Code's built-in `create-skill` skill: skill authoring process and writing guidelines.
