...
- `subagent_type`: `general-purpose`. **Do not use `Explore`.** It has no MCP access, which disables MCP-backed investigators entirely. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- `subagent_type`: `general-purpose`. The synthesizer's quality check spot-verifies citations, which can require MCP access; `Explore` has none and defeats that.
